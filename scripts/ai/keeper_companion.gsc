...
keeper_init_threaded()
{
    ...
    level.ai_companion.team = level.var_bfd9ed83.team;
    level.ai_companion setteam(level.var_bfd9ed83.team);
    level.ai_companion.maxhealth = int(level.round_number * 1000);
    level.ai_companion.health = int(level.round_number * 1000);
...
keeper_damage_ent(entity, ai, attack_origin, attack_direction, attack_type)
{
    ...
    if(isplayer(ai))
    {
        ...
        if(attack_type == "attack_up")
        {
            ai dodamage(int(level.round_number * DMG_KEEPER_ATK_UP), attack_origin, level.var_bfd9ed83, entity, "none", "MOD_UNKNOWN", 0, level.weaponnone);
        }
        else
        {
            ai dodamage(int(level.round_number * DMG_KEEPER_ATK_ELSE), attack_origin, level.var_bfd9ed83, entity, "none", "MOD_UNKNOWN", 0, level.weaponnone);
        }
