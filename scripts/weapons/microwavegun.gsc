...
weapon_is_wg(weapon)
{
    if(level.script != "zm_moon") return false;
...
microwavegun_fired(upgraded, weapon)
{
	...
	{
		player dodamage(MOON_WAVEGUN_ALTFIRE_DMG * level.round_number, self.origin, self, self, "none", "MOD_UNKNOWN", 0, level.weaponNone);
	}
}
