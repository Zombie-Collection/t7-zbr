...
flame_damage_over_time(e_attacker, damageweapon, pct_damage)
{
    ...
    e_attacker endon("spawned_player");
    e_attacker endon("bled_out");
	n_damage = get_damage_per_second(damageweapon) * level.round_number;
...
lightning_ball_fire_shot_fx(e_source, player_target)
{
    ...
    playFXOnTag(level._effect["tesla_bolt"], model, "tag_origin");
...
ice_staff_blizzard_off()
{
    ...
    level endon("blizzard_shot");
	while(isdefined(self)) wait .025;
}
...
ice_affect_zombie(str_weapon = getweapon("staff_water"), e_player, n_mod = 1)
{
	self endon("death");
    self endon("bled_out");
    self endon("disconnect");
...
whirlwind_seek_players(n_level, w_weapon)
{
	...
	while(isdefined(self))
	{
		...
		if(a_players.size)
		{
			...
			self thread whirlwind_kill_players(n_level, w_weapon, a_players);
			break;
		}
		wait(0.1);
	}
...
staff_air_players_in_range(v_source, n_range)
{
	...
	if(isdefined(a_players))
	{
		for(i = 0; i < a_players.size; i++)
		{
			...
			if(distancesquared(v_source, a_players[i].origin) > n_range_sq) continue;
...
whirlwind_drag_player(e_whirlwind, w_weapon)
{
    ...
    const_scalar = (1 + (level.round_number * STAFF_AIR_DMG_BONUS_PER_ROUND));
    dmg_last = 0;
