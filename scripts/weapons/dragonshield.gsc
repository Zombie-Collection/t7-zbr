...
collect_dragon_info()
{
	...
	players = array::get_all_closest(view_pos, level.players, undefined, undefined, level.zombie_vars["dragonshield_knockdown_range"]);
	if(!isdefined(players)) return;
    arrayremovevalue(players, self, true);
    players = array::filter(players, 0, ::gravityspikes_target_filtering);
