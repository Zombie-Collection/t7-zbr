...
bgb_stealable_trigger_check()
{
    while(1)
    {
        ...
        if(grabber == user || grabber getcurrentweapon() == level.weaponnone)
        {
            wait(0.1);
            continue;
        }
        if(grabber != user && grabber != level)
        {
            ...
            {
                grabber notify(#"hash_69873c12");
                grabber luinotifyevent(&"zombie_bgb_used", 1, level.bgb[self.var_b287be].item_index);
                grabber thread bgb::bgb_gumball_anim(self.var_b287be, 0);
